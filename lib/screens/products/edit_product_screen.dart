...
class _EditProductScreenState extends ConsumerState<EditProductScreen> {
  ...
  // Structural cascading map definitions
  ...
  // EXPRESSLY defined as Map<String, List<String>> to prevent runtime/compile type checks
  ...
  void initState() {
    ...
    selectedSubCategory = widget.product.subCategory.isNotEmpty ? widget.product.subCategory : null;
    selectedBrand = widget.product.brand.isNotEmpty ? widget.product.brand : null;
    selectedDelivery = widget.product.deliveryDuration.isNotEmpty ? widget.product.deliveryDuration : '1 - 2 Business Days'; // 👈 Added
  ...
  // ======================
  void _showVariantDialog(List<String> fields, {LocalProductVariant? existingVariant, int? index}) {
    final isEditing = existingVariant != null && index != null;

    // Pre-populate or clear standard controllers
    variantStockController.text = isEditing ? existingVariant.stock.toString() : '';
    variantWholesalePriceController.text = isEditing ? existingVariant.wholesalePrice.toString() : '';
    variantWeightController.text = isEditing ? existingVariant.weightG.toString() : '';

    // Clear and clean dynamic controllers
    for (var controller in dynamicAttributeControllers.values) {
      controller.dispose();
    }
    dynamicAttributeControllers.clear();

    // Re-initialize dynamic controllers
    for (var field in fields) {
      String initialValue = '';
      if (isEditing && existingVariant.attributes.containsKey(field)) {
        initialValue = existingVariant.attributes[field]?.toString() ?? '';
      }
      dynamicAttributeControllers[field] = TextEditingController(text: initialValue);
    }
      ...
      builder: (context) {
        return Dialog(
          child: ConstrainedBox(
            ...
            child: AlertDialog(
              title: Text(isEditing ? "Edit ${selectedCategory ?? ''} Variant" : "Add ${selectedCategory ?? ''} Variant"),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    final updatedVariant = LocalProductVariant(
  ...
  Future<void> submit() async {
    ...
    try {
      ...
      final updatedData = {
        ...
        "brand": selectedBrand ?? '',             
        "delivery_duration": selectedDelivery, // 👈 Added
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Dynamic Cascading Subcategory Dropdown Panel
                  ...
                  // Dynamic Cascading Brand Dropdown Panel
                  ...
                  // Delivery Duration Field Dropdown Panel 👈 Added
                  DropdownButtonFormField<String>(
                    value: deliveryOptions.contains(selectedDelivery) ? selectedDelivery : '1 - 2 Business Days',
                    decoration: InputDecoration(
                    ...
                    children: [
                      ...
                      TextButton.icon(
                        ...
                        onPressed: selectedCategory == null || !categoryFields.containsKey(selectedCategory)
                            ? null
                            : () => _showVariantDialog(categoryFields[selectedCategory]!),
                      ),
                    ],
                  ),
                        ...
                        return SizedBox(
                          ...
                          child: Card(
                            ...
                            child: ListTile(
                              ...
                              title: Text("Stock: ${variant.stock}"),
